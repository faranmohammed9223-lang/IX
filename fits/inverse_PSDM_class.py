...
class PSDM_Data:
    def __init__(
        ...
        self.which_compounds = which_compounds

        self.read_inputs()

    def read_inputs(self):
        """Reads in the excel data."""
        self.raw, column_data, compounds, exp_name = PSDM_functions.process_input_file(
            self.excel_file,
            data_sheet=self.data_sheet,
            column_sheet=self.column_sheet,
        )

        self.column_data = column_data[column_data.columns[0]]
        self.column_data.name = exp_name[0]
        self.compounds = (
            list(compounds) if self.which_compounds == "ALL" else list(self.compounds)
        )

        self.compound_parameters = PSDM_functions.process_input_data(
            self.excel_file, sheet_name=self.properties_sheet
        )
...
def get_mw(df, c):
    return prop(df, c, ["MW", "MolecularWeight", "Molecular Weight"])


def get_mv(df, c):
    return prop(df, c, ["MolarVol", "Molar Volume", "MolarVolume", "Vb"])


def dl(v):
    return 13.26e-5 / ((MU_CP**1.14) * (float(v) ** 0.589))


def dp(dl, t):
    return float(dl) / float(t)


def kmolar(K, MW):
    return float(K) / float(MW) / ((1 / MW) ** XN_FIXED)


def carbon(col, raw):
    if col.name in raw.columns.levels[0]:
        return col.name
...
def BV_axis(t, col):
    return float(col["flrt"]) * np.asarray(t) / BV(col)
...
def run_psdm(comp, KIX, SPDFR, raw, col, comp_data, kbase):
    ...
    model = PSDM.PSDM(
        ...
        k_data=kd,
        xn=XN_FIXED,
        xn_range=np.array([XN_FIXED]),
        test_range=np.array([KIX]),
        mass_transfer=m,
        nr=NUM_RADIAL_POINTS,
        nz=NUM_AXIAL_POINTS,
        ne=NUM_FINITE_ELEMENTS,
...
def residual(x, comp, raw, col, comp_data, kbase):
    ...
    try:
        ...
        return np.ones(len(exp["t_fit"])) * 1e6


def best_start(comp, raw, col, comp_data, kbase):
    K0 = max(float(kbase.loc["K", comp]), KIX_MIN)
    kg = np.logspace(
        np.log10(max(KIX_MIN, K0 / 100)), np.log10(min(KIX_MAX, K0 * 1000)), N_KIX_START
    )
...
def plot_best_fit(comp, exp, f, col, meta, rmse):
    ...
    plt.title(
        f"{comp} PSDM fit\nKIX={meta['KIX']:.2e}, SPDFR={meta['SPDFR']:.2f}, RMSE={rmse:.4f}",
        fontsize=12,
    )
    plt.ylim(-0.05, 1.10)
    ...
    plt.tight_layout()
    plt.savefig(f"data_out/{comp}.pdf")
    plt.close()
...
def fit(comp, raw, col, comp_data, kbase):
    ...
    res = least_squares(
        ...
        np.log10([K0, S0]),
        bounds=bnds,
        args=(comp, raw, col, comp_data, kbase),
        max_nfev=MAX_NFEV,
    )
...
if __name__ == "__main__":
    ...
    EXCEL_FILE = "data_in/experimental_6_PSDM.xlsx"
    DATA_SHEET = "data"
    ...
    data_in = PSDM_Data(
        EXCEL_FILE, DATA_SHEET, COLUMN_SHEET, PROPERTIES_SHEET, COMPOUNDS_TO_FIT
    )
    ...
    NUM_RADIAL_POINTS = 14  # nr - default 14
    NUM_AXIAL_POINTS = 19  # nz - default 19
    NUM_FINITE_ELEMENTS = 1  # ne - default 1
    SOLVER = "BDF"  # default "BDF"
    WATER_TYPE = "Organic Free"  # default "Organic Free"
    CHEM_TYPE = "PFAS"  # default "halogenated alkenes"

    kbase = PSDM.PSDM(
        data_in.column_data,
        data_in.compound_parameters,
        data_in.raw,
        compounds=data_in.compounds,
        xn=XN_FIXED,
        nr=NUM_RADIAL_POINTS,
        nz=NUM_AXIAL_POINTS,
        ne=NUM_FINITE_ELEMENTS,
    ...
    results = []
    for compound in data_in.compounds:
        results.append(
            fit(
                compound,
                data_in.raw,
