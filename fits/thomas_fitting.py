...
DATA_PATH = Path("data_in/data_cleaned.xlsx")
all_experiments = pd.read_excel(DATA_PATH, sheet_name=None)

# create output directory
out_dir = "data_out/thomas_fit"
...
for sheet_name in list(all_experiments.keys())[-12:]:
    ...
    if sheet_name in columns_to_exclude:
        df_sheet_cleaned = df_sheet_cleaned.drop(
            columns=columns_to_exclude[sheet_name], errors="ignore"
        )
    ...
    for group_name, y_columns in y_column_groups.items():
        ...
        for i, y_column_name in enumerate(y_columns):
            ...
            df_cleaned_y[x_column_name] = pd.to_numeric(
                df_cleaned_y[x_column_name], errors="coerce"
            )
            df_cleaned_y[y_column_name] = pd.to_numeric(
                df_cleaned_y[y_column_name], errors="coerce"
            )
            ...
            if y_column_name in last_row_values.index:
                ...
                BV20 = (
                    fsolve(
                        equation_bv20,
                        0,
                        args=(result.params["a"].value, result.params["b"].value),
                    )
                    * 1000
                )
                BV20_lb = (
                    fsolve(equation_bv20, 0, args=(conf_int_a[5][1], conf_int_b[1][1]))
                    * 1000
                )
                BV20_ub = (
                    fsolve(equation_bv20, 0, args=(conf_int_a[1][1], conf_int_b[5][1]))
                    * 1000
                )
                ...
                scatter = axs.scatter(
                    x_column, y_column, label=f"{y_column_name}", marker=marker
                )  # Automatic color assignment
                axs.plot(
                    x_values, y_values, color=scatter.get_facecolor()[0], linewidth=1
                )

        axs.legend(
            loc="upper right", bbox_to_anchor=(1, 1), fontsize=9
        )  # Add legend for the current figure
...
with pd.ExcelWriter(f"{out_dir}/fit_data.xlsx", engine="xlsxwriter") as writer:
        ...
        for key, value in regression_results.items():
            ...
            if current_sheet_name == sheet_name:
                ...
                if df_regression_results.empty:
                    ...
                    df_regression_results = pd.merge(
                        df_regression_results, df_temp, on="x_values", how="outer"
                    )
...
excel_file_path = f"{out_dir}/thomas_params.xlsx"
