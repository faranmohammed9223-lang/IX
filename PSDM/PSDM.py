...
kf_val = pd.read_excel(fn, sheet_name="kf_val", index_col=0)
...
flrt = float(col["flrt"])
Dl = float(np.asarray(kf_val.loc[COMPOUND, "DL (cm^2/s)"]).ravel()[0]) # liquid diffusivity
...
bv_factor = flrt / bedvol                                          # We will need this factor to convert time (minutes) to Bed Volumes (BVs)
