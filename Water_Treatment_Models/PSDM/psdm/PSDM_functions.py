...
def process_input_file(
    filename, data_sheet="data", column_sheet="column info", rept_lim=0
):
    """
    ...
    for dim in dims.keys():
        ...
        if unit == "cm":
            column_data.loc[dim] = tmp_value * 1.0
        elif unit == "mm":
            column_data.loc[dim] = tmp_value / 10.0
        elif unit == "in":
            column_data.loc[dim] = tmp_value * 2.54
        elif unit == "m":
            column_data.loc[dim] = tmp_value * 100.0
        elif unit == "ft":
    ...
    for rho in rhos.keys():
        ...
        if unit == "g/ml" or unit == "g/cm3":
            column_data.loc[rho] = tmp_value * 1.0
        elif unit == "lb/ft3":
    ...
    # unitless
    column_data.loc["tortu"] = float(c_data["value"]["tortuosity"]) * 1.0
    column_data.loc["epor"] = float(c_data["value"]["porosity"]) * 1.0
    column_data.loc["psdfr"] = float(c_data["value"]["psdfr"]) * 1.0
    ...
    units = (
        c_data["value"]["units"]
    ).lower()  # may need to add error handling for if it is under units
    ...
    if "time" in c_data.index:
        if c_data["value"]["time"] == "days" or c_data["value"]["time"] == "day":
            column_data.loc["time"] = "days"
            column_data.loc["t_mult"] = min_per_day
        elif (
            c_data["value"]["time"] == "hour"
            or c_data["value"]["time"] == "hr"
            or c_data["value"]["time"] == "hrs"
        ):
            column_data.loc["time"] = "hour"
            column_data.loc["t_mult"] = 60.0  # min_per_hour
        elif c_data["value"]["time"] == "min":
            column_data.loc["time"] = "min"
            column_data.loc["t_mult"] = 1.0  # minutes
    else:  # assume days
        column_data.loc["time"] = "days"
        column_data.loc["t_mult"] = min_per_day
    ...
    a[a < rept_lim] = 0.0  # assume zero below RepLim, LoQ, LoR
    ...
    for comp in compounds:
        infl_dict[comp] = interp1d(
            infl_idx,
            rawdata_df[influent][comp][infl_idx].values,
            fill_value="extrapolate",
        )
        effl_dict[comp] = interp1d(
            effl_idx,
    ...
    if not 0 in idx:
        idx = np.sort(idx + [0])
        rawdata_df.loc[0] = rawdata_df.loc[infl_idx[-1]] * 0.0
    ...
    rawdata_df[influent][
        rawdata_df[influent] == 0.0
    ] = 0.001  # makes this value very small, but non-zero
    ...
    print(rawdata_df, column_data, compounds, carbons)
...
def process_raw_data(filename, rept_lim=0.0):
    """
    ...
    carbons = [
        i for i in carbons if "Influent" not in i
    ]  # strip out the 'Influent' names

    # Restructure Data and Clean
    a = data_df.pivot_table(index=["time"], columns=["type", "compound"])
    a = a.fillna(0.0)
    a[a <= rept_lim] = 0.0  # min reporting limit = 10.3, assume zero
    rawdata_df = a["concentration"]
    rawdata_df[influents][
        rawdata_df[influents] == 0.0
    ] = 0.001  # makes this value very small, but non-zero
...
def calc_solver_matrix(nr, nz, ne):
    ...
    def dupoly(nd, nc, x):
        ...
        for j in range(nd):
            y[0] = 1.0
            for i in range(1, nc):
                y[i] = y[i - 1] * (2.0 * x[j] - 1.0)
            q[j, :] = np.array([y[i] for i in range(nc)])
            c[j][1] = 2.0
            for i in range(2, nc):
                c[j][i] = 2.0 * i * y[i - 1]
                d[j][i] = 4.0 * i * (i - 1.0) * y[i - 2]
...
def ohashi(re, sc):
    ...
    if re < 5.8:
        sh = 2.0 + 1.58 * (re**0.4) * (sc ** (1.0 / 3.0))
    elif (re >= 5.8) and re < 500:
        sh = 2.0 + 1.21 * (re**0.5) * (sc ** (1.0 / 3.0))
    else:
        sh = 2.0 + 0.59 * (re**0.6) * (sc ** (1.0 / 3.0))
    return sh


def gnielinski(re, sc, ebed):
    """returns sh value based on re, sc and ebed numbers"""
    sh_l = 0.644 * (re**0.5) * (sc ** (1.0 / 3.0))
    sh_t = (0.037 * (re**0.8) * (sc)) / (
        1.0 + 2.443 * (re ** (-0.1)) * ((sc ** (2.0 / 3.0)) - 1.0)
    )
    sh = (2.0 + ((sh_l**2) + (sh_t**2)) ** (0.5)) * (1.0 + 1.5 * (1.0 - ebed))
    return sh
...
def viscosity(temp):
    ...
    """
    t = temp + 273.15
    return np.exp(-24.71 + (4209.0 / t) + 0.04527 * t - (3.376e-5 * t**2)) / 100.0
...
def density(temp):
    ...
    return 0.98396 * (
        -1.41768 + 8.97665 * t - 12.2755 * t**2 + 7.45844 * t**3 - 1.73849 * t**4
    )
...
def kf_calc(multi_p, re, sc, ebed, corr="Gnielinski"):
    ...
    if corr == "Chern and Chien":
        ...
        return (
            multi_p
            * (1.0 + 1.5 * (1 - ebed))
            * (2.0 + 0.644 * (re**0.5) * (sc ** (1.0 / 3.0)))
        )
    elif corr == "Tan et al":
        # Tan et al 1975
        return multi_p * 1.1 / ebed * (re * sc) ** (1.0 / 3.0)
    elif corr == "Wilson and Geankoplis":
        # Wilson and Geankoplis, 1966
        return multi_p * 1.09 / ebed * (re * sc) ** (1.0 / 3.0)
        ...
        # Ko et al 2003
        return multi_p * (0.325 / (ebed * (re**0.36) * (sc ** (1.0 / 3.0))))
    elif corr == "Wakau and Funazkri":
        # Wakau and Funazkri, 1978
        return multi_p * (2.0 + 1.1 * (re**0.6) * (sc ** (1.0 / 3.0)))
...
def spar_Jac(ncomp, nr, nz, ne):
    ...
    low_diag[-nz_tot] = 0.0  # drops out if not nz_tot
...
def generate_grid(grid_num_xn, grid_num_k, loop_num=0, xn_range=[1], k_range=[1]):
